...
const required = [
  ...
  'TSC-09-negative-serving-directives'
];
...
assert.match(skill, /noimageindex/i);
assert.match(skill, /unavailable_after/i);
...
console.log(`PASS ${critic.practices.length} Technical SEO Critic practices, HTTP header evidence, negative-serving guardrails, obsolete-signal guardrails, canonical-channel challenge, and MetalHatsCats canonical fleet target.`);
