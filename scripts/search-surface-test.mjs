...
assert.equal(new Set(registry.checks.map(item=>item.id)).size,44);
...
assert.equal(report.registrySummary.checks,44);
...
assert.equal(merged.sourceSummary.searchSurfaceChecks,44);
...
console.log('PASS Search Surface Blueprint: 44 source-backed/conditional checks, 18 surfaces, 7 site archetypes, integrated into Site Improvement Plan without a universal score');
