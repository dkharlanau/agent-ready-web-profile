...
Canonical content/detail pages are covered by the mandatory contract in `registry/content-page-quality-contract.json` and `docs/CONTENT-PAGE-QUALITY-CONTRACT.md`, enforced through `registry/comprehensive-site-audit.json` and the `content-page-quality` final-artifact gate. This is deliberately not a separate optional skill: page-specific preview metadata, representative share images, the end-of-content Share/Copy/direct-social footer, accessible runtime fallback behavior and feedback integrity are applicability-driven requirements of complete/strict work. Priority determines remediation order; it never permits an applicable rule to be silently skipped.
...
- `arwp-internal-discovery` — improve rendered internal discovery and page distribution through canonical crawl paths, descriptive anchors, semantic/reverse relations, breadcrumbs, intentional continuation blocks, mandatory content/detail distribution controls and regression-safe graph checks without inventing a link-count ranking formula.
