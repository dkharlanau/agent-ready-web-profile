...
function configState(files, siteRoot) {
  ...
  const outputExport = /\boutput\s*:\s*['"]export['"]/.test(text);
  const outputMentioned = /\boutput\s*:/.test(text);
  ...
  return {
    ...
    outputResolved: outputExport || !outputMentioned,
...
function dynamicSegments(projectRelative, root) {
  ...
  return path.posix.dirname(projectRelative.slice(root.length + 1)).split('/').filter(segment => /\[[^\]]+\]/.test(segment));
}
...
function sourceDiagnostics(files, siteRoot, root, config) {
  ...
  if (config.deploymentMode === 'static-export') {
    ...
    if (/\b(?:output|experimental)?\s*:?\s*['"]standalone['"]/.test(config.text)) findings.push(issue('watch', 'next-static-export-mixed-output-signal', 'Static export config also contains a standalone output signal; review the effective config instead of guessing.', config.path));
  }
  ...
  for (const entry of files.values()) {
    ...
    if (config.deploymentMode === 'static-export') {
      ...
      if (/\bexport\s+const\s+revalidate\b/.test(text)) findings.push(issue('fail', 'next-static-export-isr', 'Incremental Static Regeneration/revalidate is unsupported in static export.', entry.path));
      ...
      if (isRouteHandler(entry, root)) {
        const verbs = [...text.matchAll(/\bexport\s+(?:async\s+)?function\s+(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)\b/g)].map(match => match[1]);
        const nonGet = verbs.filter(verb => verb !== 'GET');
        if (nonGet.length) findings.push(issue('fail', 'next-static-export-route-handler-non-get', `Static export Route Handlers support GET only; found ${unique(nonGet).join(', ')}.`, entry.path));
        if (/\b(?:request|req)\s*\.\s*(?:url|headers|cookies)|\bnew\s+URL\s*\(\s*(?:request|req)\.url/.test(text)) findings.push(issue('fail', 'next-static-export-route-handler-request-dependent', 'Route Handler reads incoming Request data, which cannot be resolved by static export.', entry.path));
      }
    }
...
export function compileNextjsSearchPack(options = {}) {
  ...
  const sourceWatches = source.findings.filter(item => item.severity === 'watch');
