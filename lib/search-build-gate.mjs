...
function parseHtmlSignals(html, pageUrl) {
  ...
  const headMatch = document.match(/<head\b[^>]*>([\s\S]*?)<\/head\s*>/i);
  ...
  let jsonLdCount = 0;
  const jsonLdDigests = [];
  for (const match of document.matchAll(/<script\b[^>]*type\s*=\s*(?:"application\/ld\+json"|'application\/ld\+json'|application\/ld\+json)[^>]*>([\s\S]*?)<\/script\s*>/gi)) {
    jsonLdCount += 1;
    try {
      const parsed = JSON.parse(match[1]);
      jsonLdDigests.push(sha256(JSON.stringify(canonicalJson(parsed))));
  ...
  return {
    ...
    jsonLdDigests: jsonLdDigests.sort()
  };
}
...
export async function verifySearchArtifactLive(manifest, {
  ...
  return {
    ...
    revision,
    pages,
    pass: failures === 0 && coverage === 'complete-index-cohort' && revision.state === 'pass',
    failures,
