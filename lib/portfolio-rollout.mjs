...
function stageSet(options) {
  const stages = options.stage ? csvSet(options.stage) : new Set(DEFAULT_STAGES);
  stages.delete('retired');
  if (!options.includeWatch) stages.delete('watch');
  else if (!options.stage) stages.add('watch');
  return stages;
}
...
function verticalMatch(site, trend) {
  ...
  return { applies: matchedVerticals.length > 0, matchedVerticals };
}
...
function candidateReason(site, trend, match) {
  return `${trend.stage.toUpperCase()} ${trend.provider} trend mapped to ${site.name} because matched vertical${match.matchedVerticals.length === 1 ? '' : 's'}: ${match.matchedVerticals.join(', ')}. This is a rollout candidate only; the target site must still be audited before any change is proposed or applied.`;
}
