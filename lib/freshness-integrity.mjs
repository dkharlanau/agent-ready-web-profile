...
export function parseLeafSitemap(xml) {
  ...
  for (const match of text.matchAll(/<url\b[^>]*>([\s\S]*?)<\/url>/gi)) {
    ...
    const lastmodState = lastmod == null ? 'missing' : Number.isNaN(Date.parse(lastmod)) ? 'invalid' : 'valid';
...
export function compareFreshnessSnapshots(before, after, options = {}) {
  ...
  for (const url of urls) {
    ...
    const lastmodChanged = left.lastmod !== right.lastmod;
    ...
    if (!lastmodChanged && ownerChanged) {
      const reason = 'The exact mapped route owner changed while sitemap lastmod stayed unchanged. The source change may be insignificant, so this requires review rather than automatic date mutation.';
      ...
      actions.push(reviewAction(
        ...
        'Review the owner-file diff and rendered page. Update lastmod only if the change is significant to page content, structured data or links; do not change it for cosmetic/build-only edits.'
      ));
      continue;
    }
    if (lastmodChanged && (ownerChanged || mappedInputsChanged)) {
