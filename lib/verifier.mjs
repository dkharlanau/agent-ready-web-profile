...
import { probeMcpEndpoint } from './mcp-runtime.mjs';
...
function isStreamableHttpMcpUrl(item) {
  return item.key.startsWith('mcp.servers.') && item.key.endsWith('.url') && item.transport === 'streamable-http';
}

async function probeMcpServerResource(item, { fetchImpl, resolveImpl, timeoutMs, maxBytes }) {
  const started = Date.now();
  let runtime;
  try {
    runtime = await probeMcpEndpoint(item.url, { fetchImpl, resolveImpl, timeoutMs, maxBytes });
  } catch (error) {
    return {
      ...item,
      status: 'fail',
  ...
  const base = {
    ...item,
    method: 'MCP',
    finalUrl: runtime.endpoint || item.url,
    httpStatus: runtime.httpStatus,
    mcp: {
      era: runtime.era ?? null,
      discoverSupported: runtime.discoverSupported ?? null,
  ...
  if (runtime.status === 'runtime-observed') {
    return { ...base, status: 'pass', issues: [], verification: 'protocol-verified' };
  }
  if (runtime.status === 'authorization-required') {
    return {
      ...base,
      status: 'warn',
      issues: [`MCP endpoint requires authorization (HTTP ${runtime.httpStatus}); protocol verification not assessed`],
      verification: 'not-assessed'
    };
  }
  const evidence = runtime.error
    ? (typeof runtime.error === 'string' ? runtime.error : JSON.stringify(runtime.error))
    : `HTTP ${runtime.httpStatus ?? 'error'}`;
  return {
    ...base,
    status: 'fail',
    issues: [`MCP protocol verification failed: ${evidence}`],
    verification: 'failed'
  };
}
...
export async function verifyProfileSource(source, {
  ...
  async function worker() {
    while (true) {
      ...
      resources[index] = isStreamableHttpMcpUrl(item)
        ? await probeMcpServerResource(item, { fetchImpl, resolveImpl, timeoutMs, maxBytes })
        : await probeResource(item, { fetchImpl, timeoutMs });
    }
  }
