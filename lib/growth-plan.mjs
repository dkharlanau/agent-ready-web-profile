...
export function refineGrowthPlan(plan, { localSitemap = null } = {}) {
  ...
  if (!contentSignalApplicable) {
    actions = actions.filter(item => item.id !== 'growth:cloudflare-content-signals');
  }
  ...
  return {
    ...
    refinements: {
      ...
      note: 'The Growth backlog is intentionally smaller than the raw audit. External/runtime-only observations remain in the audit rather than becoming repetitive implementation tasks. Provider-specific actions are emitted only when public evidence makes that provider applicable.'
    }
  };
}
