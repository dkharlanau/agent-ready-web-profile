...
function isLikelyJsonFeedFallback(link, href) {
  if (mediaTypeOnly(link?.type) !== 'application/json') return false;
  const title = decodeHtml(link?.title || '').toLowerCase();
  const pathname = new URL(href).pathname.toLowerCase();
  return /\bjson\s*feed\b/.test(title)
    || /(?:^|\/)(?:feed\.json|jsonfeed(?:\.json)?|feeds?\/json)\/?$/.test(pathname);
}
...
function extractDocumentMetadata(html, baseUrl, warnings) {
  ...
  for (const link of links) {
    ...
    if (rel.includes('alternate') && (FEED_MEDIA_TYPES.has(type) || isLikelyJsonFeedFallback(link, href))) {
