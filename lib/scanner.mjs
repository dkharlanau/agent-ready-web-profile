...
async function fetchTextResource(url, options) {
  ...
  const text = await readTextBounded(response, options.maxBytes);
  return {
    ok: true,
    status: response.status,
    url: finalUrl,
...
export async function scanSite(input, {
  ...
  const homepage = await fetchTextResource(source, { ...requestOptions, accept: 'text/html, application/xhtml+xml;q=0.9, */*;q=0.1' });
