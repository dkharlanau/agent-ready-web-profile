...
export function analyzeInternalDiscoveryFromPages({ canonicalUrl, pages, cohort = null, generatedAt = new Date().toISOString() }) {
  ...
  const nodesByRequestedUrl = new Map(observed.map(node => [normalizeUrl(node.requestedUrl), node]));
  const rawEdges = [];
  for (const node of owners) {
    for (const link of node._links) {
      const target = normalizeUrl(link.to);
      const canonicalOwnerTarget = ownersByUrl.get(target);
      const requestedTarget = nodesByRequestedUrl.get(target);
      let targetState = 'out-of-cohort';
      if (canonicalOwnerTarget) targetState = 'canonical-owner';
      else if (requestedTarget && requestedTarget.nodeClass !== 'unknown') targetState = 'transition';
      else if (requestedTarget?.nodeClass === 'unknown') targetState = 'unknown';
      rawEdges.push({
