...
function selectedResponseHeaders(response) {
  return {
    contentUsage: response.headers?.get?.('content-usage') || null,
...
export async function probePublicHttpsUrl(url, {
  fetchImpl = fetch,
  ...
  for (let redirectCount = 0; redirectCount <= maxRedirects; redirectCount += 1) {
    ...
    if (response.status >= 300 && response.status < 400 && response.status !== 304) {
      ...
      if (redirectCount === maxRedirects) throw new Error(`Too many redirects while probing ${url}`);
    ...
    return {
      ...
      url: current.href,
      redirected: requested.href !== current.href,
