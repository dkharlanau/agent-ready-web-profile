...
function targetNode(graph, revision) {
  if (revision.entityType === 'rule') {
    return graph.nodes.find(node => node.type === 'rule' && node.data?.ruleId === revision.currentRef && !node.data?.historical) || null;
  }
  if (revision.entityType === 'source') {
    return graph.nodes.find(node => node.type === 'source' && node.data?.url === revision.currentRef && !node.data?.historical) || null;
  }
...
function nodeSort(a, b) {
  ...
  const historyOrder = Number(Boolean(a.data?.historical)) - Number(Boolean(b.data?.historical));
...
export function enrichBraidHistory(inputGraph, revisionRecords = []) {
  ...
  // BraidGraph's human selectors resolve the first matching source/rule node.
  // Keep the current node before historical versions while retaining stable IDs/provenance.
  graph.nodes = [...nodes.values()].sort(nodeSort);
