...
import { analyzeAgentAccessibilitySite } from './agent-accessibility.mjs';
...
function agentAccessibilityAction(item) {
  return {
    id:item.id,
    sourceKind:'page-graph',
    priority:item.priority || 'P2',
    lane:item.lane || 'agent-accessibility',
    title:item.title,
    reason:item.reason,
    evidenceClass:item.evidenceClass || 'manual-review',
    evidence:item.evidence || [],
    target:item.target || null,
    proposal:null,
    verification:item.verification || ['Verify the rendered interaction with accessibility tooling and a browser-agent runtime task.'],
    measurement:item.measurement || ['Preserve runtime task evidence; interactive semantics are not a Search ranking signal.'],
    dependencies:['runtime-agent-accessibility-review'],
    sourceCheck:item.sourceCheck || null
  };
}
...
export function mergeSearchSurfacePlan(base,surfaces,{maxActions=8,contentDifferentiation=null,agentAccessibility=null}={}) {
  const candidates=dedupe([
    ...base.actions.map(({order,...item})=>item),
    ...surfaces.actions.map(surfaceAction),
    ...(contentDifferentiation?.actions||[]).map(contentAction),
    ...(agentAccessibility?.actions||[]).map(agentAccessibilityAction)
  ]);
  ...
  return {
    ...
    sourceSummary:{
      ...
      ...(agentAccessibility?{
        agentAccessibilityVersion:agentAccessibility.version,
        agentAccessibilityPages:agentAccessibility.summary.pagesObserved,
        agentAccessibilityInteractivePages:agentAccessibility.summary.interactivePagesObserved,
        agentAccessibilityActions:agentAccessibility.actions.length
      }:{})
    },
    searchSurface:{
    ...
    ...(agentAccessibility?{
      agentAccessibility:{
        version:agentAccessibility.version,
        scope:agentAccessibility.scope,
        pagesObserved:agentAccessibility.summary.pagesObserved,
        interactivePagesObserved:agentAccessibility.summary.interactivePagesObserved,
        actions:agentAccessibility.summary.actions,
        source:agentAccessibility.source,
        guardrails:agentAccessibility.guardrails
      }
    }:{}),
    ...
    guardrails:{
      ...
      agentAccessibilityIsRuntimeReviewNotRankingSignal:true,
      staticAriaReviewDoesNotClaimWcagCompliance:true
    }
  };
}
...
export async function buildSiteImprovementPlan(input, options={}) {
  ...
  const [base,surfaces,contentDifferentiation,agentAccessibility]=await Promise.all([
    ...
    analyzeAgentAccessibilitySite(input,{
      timeoutMs:options.timeoutMs||8000,
      maxBytes:options.maxBytes||256*1024,
      maxPages:Math.min(options.maxPages||12,6),
      fetchImpl:options.fetchImpl||fetch,
      ...(options.resolveImpl?{resolveImpl:options.resolveImpl}:{})
    })
  ]);
  return mergeSearchSurfacePlan(base,surfaces,{maxActions,contentDifferentiation,agentAccessibility});
}
...
export function formatSiteImprovementPlan(plan) {
  ...
  const accessibility=plan.agentAccessibility;
  return `${body}\n\nSearch Surface Blueprint: ${kind||'unknown'}; observed surfaces: ${observed||'none'}. Optional surfaces remain conditional.${differentiation?`\nContent differentiation: pages=${differentiation.pagesObserved}; informational=${differentiation.informationalPagesObserved}; review-actions=${differentiation.actions}. Observable review signals only; no content-quality score.`:''}${accessibility?`\nAgent accessibility: pages=${accessibility.pagesObserved}; interactive=${accessibility.interactivePagesObserved}; review-actions=${accessibility.actions}. Static review only; verify rendered browser-agent behavior.`:''}`;
}
