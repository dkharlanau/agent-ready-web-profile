...
async function checkSource(source, options) {
  if (source.kind === 'manual-page') {
    return {
      sourceId: source.id,
      provider: source.provider,
      kind: source.kind,
      url: source.url,
      canonicalPage: source.canonicalPage || source.url,
      ok: null,
      manual: true,
      status: null,
      reviewedThrough: source.reviewedThrough,
      candidates: [],
      note: source.reason || 'This source requires explicit manual primary-source review.'
    };
  }

  const base = await fetchText(source.url, options);
  if (!base.ok) return { sourceId: source.id, provider: source.provider, kind: source.kind, url: source.url, ok: false, manual: false, status: base.status, candidates: [], error: `HTTP ${base.status}` };
  ...
  return {
    ...
    kind: source.kind,
    url: source.url,
    canonicalPage: source.canonicalPage || null,
...
export async function runTrendSourceWatch(config = loadTrendWatchConfig(), options = {}) {
  ...
  for (const source of config.sources || []) {
    try {
      ...
      results.push({ sourceId: source.id, provider: source.provider, kind: source.kind, url: source.url, ok: false, manual: false, status: null, candidates: [], error: String(error?.message || error) });
    }
  }
  ...
  return {
    ...
    summary: {
      ...
      reachable: results.filter(item => item.ok === true && !item.manual).length,
      manual: results.filter(item => item.manual).length,
      failed: results.filter(item => item.ok === false && !item.manual).length,
      candidates: candidates.length
    },
    candidates,
    results,
    guardrails: config.guardrails,
    note: 'A candidate is only a prompt for source review. Manual sources remain visible and must not be reported as transport failures. No source change automatically creates or promotes an ARWP recommendation.'
  };
}
