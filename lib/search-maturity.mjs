  ...
  for (const observation of corpus.observations || []) {
    ...
    for (const dimension of SEARCH_MATURITY_DIMENSIONS) {
      ...
      if (typeof feature !== 'object') {
        errors.push(observationError(id, `features.${dimension} must be an object when provided`));
        continue;
      }
...
export function buildSearchMaturityCohort(corpus, { intentFamily, ownership = 'independent', floorShare = 0.6 } = {}) {
  ...
  const dimensions = {};
  for (const dimension of SEARCH_MATURITY_DIMENSIONS) {
    const features = selected.map((observation) => observation.features[dimension] || { state: 'unknown', confidence: 'low', evidenceClass: 'unknown', observations: [], sources: [] });
...
export function compareTargetToSearchMaturityCohort(targetProfile, cohort) {
  ...
  for (const dimension of SEARCH_MATURITY_DIMENSIONS) {
    const target = targetProfile.features[dimension] || { state: 'unknown' };
    if (!SEARCH_MATURITY_STATES.includes(target.state)) throw new Error(`targetProfile.features.${dimension}.state must be valid when provided`);
