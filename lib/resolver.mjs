...
import { fetchPublicJson, fetchPublicText, mediaTypeOnly } from './public-fetch.mjs';
import {
  emptyInterfaces,
  mergeInterfaces,
  dedupeInterfaces,
...
function attachAuthority(interfaces, sourceMap) {
  for (const values of Object.values(interfaces)) {
    for (const item of values) item.sourceAuthority = sourceMap.get(item.sourceId)?.authority || 'unknown';
  }
}
...
export async function resolveSite(input, {
  ...
  if (scan.existingProfile?.url) {
    const arwp = await tryJson(scan.existingProfile.url, 'arwp-profile', 0, state, options, (payload, id) => {
    ...
    if (arwp?.identity) state.identityEvidence.push({ sourceId: 'arwp-profile:0', ...arwp.identity });
  }

  const agentsJson = await tryJson(new URL('/agents.json', origin).href, 'agents-json', 0, state, options, adaptAgentsJson);
  if (agentsJson?.identity) state.identityEvidence.push({ sourceId: 'agents-json:0', ...agentsJson.identity });
  ...
  await tryJson(new URL('/.well-known/api-catalog', origin).href, 'api-catalog', 0, state, { ...options, accept: 'application/linkset+json, application/json;q=0.9, */*;q=0.1' }, adaptApiCatalog);
  ...
  const resolvedIdentity = state.identityEvidence.find(item => item.sourceId === 'arwp-profile:0') || state.identityEvidence.find(item => item.sourceId === 'agents-json:0') || state.identityEvidence[0];
  const resolution = {
    resolverVersion: '0.1',
    target: input,
