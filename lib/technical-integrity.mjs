...
export const TECHNICAL_INTEGRITY_VERSION = '0.1';
...
function combinedDirectives(page, agent) {
  return new Set([
    ...metaDirectives(page.html, 'robots'),
    ...metaDirectives(page.html, agent),
    ...headerDirectives(page.headers, agent)
  ]);
}
...
function observePage(page) {
  ...
  const canonical = canonicalSignals(page.html, finalUrl);
  const googleDirectives = combinedDirectives(page, 'googlebot');
  const bingDirectives = combinedDirectives(page, 'bingbot');
  const text = cleanText(mainHtml(page.html));
  const links = linkSignals(page.html, finalUrl);
  const hreflang = hreflangSignals(page.html, finalUrl);
  const dataNosnippetCount = (String(page.html).match(/\bdata-nosnippet\b/gi) || []).length;
  const scriptCount = (String(page.html).match(/<script\b/gi) || []).length;
  const h1Count = (String(page.html).match(/<h1\b/gi) || []).length;
  return {
    requestedUrl: page.requestedUrl,
    url: finalUrl,
    ok: true,
    ...
    links,
    hreflang,
    metaRefresh: /<meta\b[^>]*http-equiv\s*=\s*["']?refresh["']?[^>]*>/i.test(String(page.html)),
    _text: text
  };
}
...
function indexabilityFinding(rule, pages) {
  const failures = pages.filter(page => !page.ok || page.status < 200 || page.status >= 300 || page.noindex);
  if (failures.length) {
    return finding(rule, 'fail', `${failures.length}/${pages.length} sampled priority page(s) fail the public indexability integrity check (non-success response and/or noindex).`, failures.map(page => page.url || page.requestedUrl));
  }
...
function canonicalFinding(rule, pages) {
  const hard = [];
  const review = [];
  for (const page of pages.filter(item => item.ok)) {
  ...
  if (hard.length) return finding(rule, 'fail', hard.slice(0, 8).join('; '), pages.filter(page => page.ok).map(page => page.url), { issueCount: hard.length, reviewCount: review.length });
  if (review.length) return finding(rule, 'watch', review.slice(0, 8).join('; '), pages.filter(page => page.ok).map(page => page.url), { reviewCount: review.length });
  return finding(rule, 'pass', 'Each successful sampled page exposes one valid head canonical matching its observed final URL; no meta refresh was observed.', pages.filter(page => page.ok).map(page => page.url));
}

function collisionFinding(rule, pages) {
  const clusters = new Map();
  for (const page of pages.filter(item => item.ok && item.canonical)) {
    const key = normalizeUrl(page.canonical);
    const values = clusters.get(key) || [];
    values.push(page.url);
    clusters.set(key, values);
  }
  const collisions = [...clusters.entries()].filter(([, values]) => new Set(values).size > 1);
  if (collisions.length) {
    return finding(rule, 'watch', `${collisions.length} sampled canonical cluster(s) contain multiple distinct priority URLs. Confirm consolidation is intentional rather than route duplication.`, collisions.flatMap(([canonical, values]) => [canonical, ...values]).slice(0, 20), { collisionCount: collisions.length });
  }
  return finding(rule, 'pass', 'No duplicate canonical target was observed across the sampled priority cohort.', pages.filter(page => page.ok).map(page => page.url));
}

function linkFinding(rule, pages) {
  const risky = pages.filter(page => page.ok && page.links?.anchors > 0 && page.links.pseudoLinks > 0);
  const noInternal = pages.filter(page => page.ok && page.links?.anchors > 0 && page.links.internalCrawlable === 0);
  ...
  return finding(rule, 'pass', 'No obvious href-less/javascript anchor problem was observed on sampled pages, and pages with anchors expose crawlable same-origin hrefs.', pages.filter(page => page.ok).map(page => page.url));
}

function textualFinding(rule, pages) {
  const shellRisk = pages.filter(page => page.ok && page.textWords < 80 && page.scriptCount >= 3);
  const empty = pages.filter(page => page.ok && page.textWords < 20);
  if (empty.length) return finding(rule, 'watch', `${empty.length}/${pages.length} sampled page(s) expose fewer than 20 meaningful text words in raw public HTML. Verify rendered content and critical text availability with provider/browser tooling.`, empty.map(page => page.url));
  if (shellRisk.length) return finding(rule, 'watch', `${shellRisk.length}/${pages.length} sampled page(s) expose fewer than 80 raw-text words while loading at least 3 scripts. This is a JS-shell risk signal, not proof that rendered content is unavailable.`, shellRisk.map(page => page.url));
  return finding(rule, 'pass', 'No sampled page matched the bounded thin-raw-HTML/script-shell risk heuristic.', pages.filter(page => page.ok).map(page => page.url));
}

function hreflangFinding(rule, pages) {
  const withHreflang = pages.filter(page => page.ok && page.hreflang?.length);
  if (!withHreflang.length) return finding(rule, 'not-applicable', 'No hreflang annotations were observed in the sampled cohort.');
  const byUrl = new Map(pages.filter(page => page.ok).map(page => [normalizeUrl(page.url), page]));
  const issues = [];
  ...
  return finding(rule, 'pass', `${withHreflang.length} sampled page(s) use hreflang with valid self references and reciprocal links for alternate targets that were also inside the sampled cohort.`, withHreflang.map(page => page.url));
}
...
function duplicateFinding(rule, pages) {
  ...
  for (const page of ready) if (page.title) {
    const key = page.title.toLowerCase();
    const values = titleMap.get(key) || [];
    values.push(page.url);
    titleMap.set(key, values);
  }
  const duplicateTitles = [...titleMap.entries()].filter(([, values]) => values.length > 1);
  if (pairs.length || duplicateTitles.length) {
    return finding(rule, 'watch', `Near-duplicate review: ${pairs.length} content pair(s) at >=0.92 five-word-shingle similarity and ${duplicateTitles.length} repeated title cluster(s). This is a route/template risk signal, not a spam verdict.`, [
      ...pairs.flatMap(pair => [pair.a, pair.b]),
      ...duplicateTitles.flatMap(([, values]) => values)
    ], { nearDuplicatePairs: pairs, duplicateTitleClusters: duplicateTitles.length });
  }
  return finding(rule, 'pass', 'No >=0.92 near-duplicate content pair or repeated non-empty title cluster was observed in the sampled priority cohort.', ready.map(page => page.url));
}
...
function openaiFinding(rule, pages, robots) {
  if (robots?.ok) {
    ...
    if (oai.status === 'blocked') return finding(rule, 'watch', `OAI-SearchBot appears root-blocked while GPTBot=${gpt.status}. This can be intentional, but it conflicts with a goal of inclusion in ChatGPT summaries/snippets.`, [robots.url]);
    return finding(rule, 'pass', `No root-wide OAI-SearchBot block was observed. GPTBot=${gpt.status}; Search discovery and training policy remain separate.`, [robots.url]);
  }
...
export async function technicalIntegrity(input, {
  ...
  const robotsResponse = await fetchPublicText(robotsUrl, {
    ...
    userAgent: 'goose-technical-integrity/0.1'
