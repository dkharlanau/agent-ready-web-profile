...
function openaiFinding(rule, pages, robots) {
  if (robots?.ok) {
    ...
    const blockedPaths = pages.filter(page => robotsPathAccess(robots.text, 'OAI-SearchBot', page.url || page.requestedUrl).status === 'blocked');
    if (blockedPaths.length) return finding(rule, 'watch', `${blockedPaths.length}/${pages.length} sampled priority URL(s) appear blocked for OAI-SearchBot. GPTBot=${gpt.status}; Search discovery and training policy remain separate.`, [robots.url, ...blockedPaths.map(page => page.url || page.requestedUrl)]);
    if (root.status === 'blocked') return finding(rule, 'watch', `OAI-SearchBot appears root-blocked while GPTBot=${gpt.status}. This can be intentional, but it conflicts with a goal of inclusion in ChatGPT summaries/snippets.`, [robots.url]);
    return finding(rule, 'pass', `No root-wide or sampled path-specific OAI-SearchBot block was observed. GPTBot=${gpt.status}; Search discovery and training policy remain separate.`, [robots.url]);
  }
