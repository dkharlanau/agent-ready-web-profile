...
function desiredVerticalModule(policy) {
  const vertical = verticalRegistry.verticals[policy.siteClass] || verticalRegistry.verticals.general;
  return {
    registryVersion: verticalRegistry.version,
    reviewedAt: verticalRegistry.reviewedAt,
    focus: Array.isArray(vertical.focus) ? vertical.focus : [],
    checks: Array.isArray(vertical.checks) ? vertical.checks : [],
    notes: vertical.notes || null
  };
}
...
export function compileGrowthPolicy(plan, policy) {
  ...
  return {
    ...
    goals: policy.goals,
    desiredState: {
      vertical: desiredVerticalModule(policy),
...
export function formatImplementationManifest(value) {
  ...
  if (value.desiredState.vertical?.checks?.length) {
    lines.push('', `Vertical evidence checks (${value.siteClass}):`);
    for (const check of value.desiredState.vertical.checks) {
      lines.push(`- ${check.priority} ${check.title}`);
