...
export const MODULE_DEFAULTS = Object.freeze({
  ...
  openReuseAssets: ['P1', 'Publish a permission-free press/media pack with machine-readable reuse rights, canonical facts and attribution guidance, including explicit AI/ML reuse policy.'],
  trustCenter: ['P0', 'Publish one canonical trust surface covering maintainer identity, evidence policy, security, provenance, corrections, reuse and bounded limitations.'],
  correctionsLedger: ['P1', 'Maintain a public corrections policy and machine-readable ledger that records material published factual errors without silently rewriting history.'],
  softwareProvenance: ['P1', 'Publish and operate verifiable software build/publish provenance for artifacts people actually consume, while keeping provenance distinct from security claims.'],
  persistentIdentifiers: ['P1', 'Use externally issued persistent identifiers such as DOI when they materially improve durable citation, and never claim identifiers before issuance.'],
  externalTrustSignals: ['P2', 'Expose independent or externally generated trust signals such as OpenSSF results while keeping their scope and limitations explicit.'],
...
export function createAiSearchProfileStarter(input, {
  ...
  return {
    ...
    surfaces: {
      ...
      pressKit: plannedSurface(new URL('media/', base).href, { mediaType: 'text/html' }),
      reuseRights: plannedSurface(new URL('media/rights.json', base).href, { mediaType: 'application/json' }),
      trustCenter: plannedSurface(new URL('trust/', base).href, { mediaType: 'text/html' }),
      correctionsLedger: plannedSurface(new URL('trust/corrections.json', base).href, { mediaType: 'application/json' })
    },
    modules: modulePlan(),
    vocabulary: [],
    measurement: {
