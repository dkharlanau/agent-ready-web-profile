...
import { auditProjectSurfaces } from './project-surfaces.mjs';
...
export function auditMaturity(profile, { readText } = {}) {
  ...
  if (profile.projectSurfaces !== undefined) {
    require(object(profile.projectSurfaces), 'projectSurfaces must be an object when present');
    if (object(profile.projectSurfaces)) {
      require(safeRelativePath(profile.projectSurfaces.manifest), 'projectSurfaces.manifest must be a repository-relative JSON path');
      const manifest = safeRelativePath(profile.projectSurfaces.manifest) ? json(profile.projectSurfaces.manifest) : null;
      if (manifest !== null) {
        const projectAudit = auditProjectSurfaces(manifest, { readText: file => {
          const content = read(file);
          if (content === null) throw new Error('unreadable project surface');
          return content;
        } });
        for (const error of projectAudit.errors) errors.push(`Project surfaces: ${error}`);
        for (const note of projectAudit.review) review.push(`Project surfaces: ${note}`);
      }
    }
  }
