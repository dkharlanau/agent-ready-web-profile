...
function valueContainsCanonical(value, pageUrl) {
  const canonical = normalizeUrl(pageUrl);
  if (!canonical) return false;
  const raw = String(value || '').trim();
  if (!raw) return false;
  if (sameUrl(raw, canonical)) return true;

  const variants = new Set([canonical]);
  if (canonical.endsWith('/')) variants.add(canonical.slice(0, -1));
  else variants.add(`${canonical}/`);
  return [...variants].some(candidate => candidate && raw.includes(candidate));
}

function providerLinkValid(item, pageUrl) {
  if (!item || item.tagName !== 'a') return false;
  const href = isHttpsUrl(item.attrs?.href);
  if (!href || sameUrl(href, pageUrl)) return false;

  const providerUrl = new URL(href);
  const canonicalUrl = new URL(pageUrl);
  if (providerUrl.origin === canonicalUrl.origin) return false;

  return [...providerUrl.searchParams.values()].some(value => valueContainsCanonical(value, pageUrl));
}
...
export function inspectContentPageQuality({ html, url }) {
  ...
  const providerLinksValid = distribution.providers.length > 0 && distribution.providers.every(item => providerLinkValid(item, pageUrl));
  const invalidProviders = distribution.providers
    .filter(item => !providerLinkValid(item, pageUrl))
    .map(item => item.provider);
  const footerOk = distribution.footers.length === 1;
  const providerCountOk = distribution.uniqueProviders.length >= 2;
  const distributionOk = footerOk && shareNamesOk && copyNamesOk && providerNamesOk && providerCountOk && providerLinksValid;
  checks.push(check(
    'CPQ-05-end-of-content-sharing',
    'P0',
    distributionOk ? 'pass' : 'fail',
    distributionOk
      ? `Distribution footer exposes Share, Copy link and ${distribution.uniqueProviders.length} valid direct provider targets carrying the canonical URL: ${distribution.uniqueProviders.join(', ')}.`
      : `Distribution footer incomplete or invalid: footers=${distribution.footers.length}, share=${distribution.share.length}, copy=${distribution.copy.length}, directProviders=${distribution.uniqueProviders.length}, providerLinksValid=${providerLinksValid}${invalidProviders.length ? `, invalidProviders=${invalidProviders.join(',')}` : ''}.`,
    distribution.uniqueProviders
  ));

  const accessibilitySignal = distributionOk;
  checks.push(check(
    'CPQ-06-sharing-accessibility-and-resilience',
    'P0',
    accessibilitySignal ? 'pass' : 'fail',
    accessibilitySignal
      ? 'Static controls expose accessible-name and valid-provider-link signals. Keyboard/focus/Web Share failure behavior still needs rendered runtime evidence.'
      : 'One or more required share/copy/provider controls are absent, lack an accessible name, or expose an invalid direct provider target.',
  ...
  return {
    ...
    observations: {
      ...
      invalidProviderTargets: invalidProviders,
