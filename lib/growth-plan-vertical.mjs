...
import { applySearchPlatformEligibilityToGrowthPlan } from './growth-platform-eligibility.mjs';
...
export async function buildGrowthPlan(input, options = {}) {
  ...
  const base = applySearchPlatformEligibilityToGrowthPlan(
    await baseBuilder(input, options),
    { siteScope: options.siteScope || 'unknown' }
  );
