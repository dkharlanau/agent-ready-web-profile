...
const DEFAULT_RESOLVER_MAX_BYTES = 1024 * 1024;
...
function pathScopedLlmsUrl(canonicalUrl) {
  const canonical = new URL(canonicalUrl);
  if (canonical.pathname === '/') return null;
  let basePath = canonical.pathname;
  if (!basePath.endsWith('/')) basePath = basePath.replace(/[^/]*$/, '');
  if (!basePath || basePath === '/') return null;
  return new URL(`${basePath}llms.txt`, canonical.origin).href;
}
...
async function tryJson(url, type, sourceIndex, state, options, adapt) {
  ...
  try {
    ...
    if (adapted?.valid === false) throw new Error((adapted.issues || ['Artifact failed minimum shape validation.']).join(' '));
...
async function mergeArdCards(catalog, state, options, cardState) {
  if (!catalog?.cards?.length) return;
  for (const card of catalog.cards.slice(0, 8)) {
    if (cardState.count >= 12) break;
    if (card.data) {
      const id = makeSourceId('mcp-server-card-inline', cardState.count++);
      const adapted = adaptMcpServerCard(card.data, id, catalog.fetched.url);
      state.sources.push(sourceRecord(id, 'mcp-server-card-inline', catalog.fetched.url, 'resolved', { authority: adapted.authority, identifier: card.identifier, discoveredVia: 'ARD' }));
      state.resolved += 1;
      state.attempted += 1;
      state.sourceInterfaces.set(id, adapted.interfaces);
      mergeInterfaces(state.interfaces, adapted.interfaces);
    } else if (card.url) {
      await tryJson(card.url, 'mcp-server-card', cardState.count++, state, { ...options, accept: 'application/mcp-server-card+json, application/json;q=0.9, */*;q=0.1' }, (payload, id, url) => adaptMcpServerCard(payload, id, url));
    }
  }
}
...
export async function resolveSite(input, {
  ...
  timeoutMs = 8000,
  maxBytes = DEFAULT_RESOLVER_MAX_BYTES
} = {}) {
  ...
  const scopedLlms = pathScopedLlmsUrl(scan.canonicalUrl);
  const alreadyObservedLlms = new Set(state.interfaces.content.filter(item => item.kind === 'llms').map(item => normalizeUrl(item.url)));
  if (scopedLlms && !alreadyObservedLlms.has(normalizeUrl(scopedLlms))) {
    await tryText(scopedLlms, 'path-llms', 0, state, { ...options, accept: 'text/plain, text/markdown;q=0.9, */*;q=0.1' }, (_text, id, url) => ({
      authority: 'observed-web',
      interfaces: { ...emptyInterfaces(), content: [{ sourceId: id, sourceAuthority: 'observed-web', kind: 'llms', protocol: 'llms.txt', url }] }
    }));
  }
  ...
  // ARD v0.91 makes /.well-known/ard.json canonical. The predecessor ai-catalog path remains a compatibility source.
  const canonicalArdUrl = new URL('/.well-known/ard.json', origin).href;
  const legacyAiCatalogUrl = new URL('/.well-known/ai-catalog.json', origin).href;
  const ardResults = [];
  const canonicalArd = await tryJson(canonicalArdUrl, 'ard-catalog', 0, state, { ...options, accept: 'application/ld+json, application/json;q=0.9, */*;q=0.1' }, (payload, id) => ({ interfaces: emptyInterfaces(), ...adaptAiCatalog(payload, id) }));
  if (canonicalArd) ardResults.push(canonicalArd);

  const linkedArdUrls = [...new Set(httpDiscovery?.ardCatalogs || [])]
    .filter(url => ![normalizeUrl(canonicalArdUrl), normalizeUrl(legacyAiCatalogUrl)].includes(normalizeUrl(url)))
    .slice(0, 3);
  for (let index = 0; index < linkedArdUrls.length; index += 1) {
    const linked = await tryJson(linkedArdUrls[index], 'ard-catalog-link', index, state, { ...options, accept: 'application/ld+json, application/json;q=0.9, */*;q=0.1' }, (payload, id) => ({ interfaces: emptyInterfaces(), ...adaptAiCatalog(payload, id) }));
    if (linked) ardResults.push(linked);
  }

  // Avoid a second conventional fetch when the canonical manifest resolved. A linked predecessor URL remains discoverable evidence through HTTP metadata.
  if (!canonicalArd) {
    const legacy = await tryJson(legacyAiCatalogUrl, 'ard-catalog-legacy', 0, state, { ...options, accept: 'application/ai-catalog+json, application/json;q=0.9, */*;q=0.1' }, (payload, id) => ({ interfaces: emptyInterfaces(), ...adaptAiCatalog(payload, id) }));
    if (legacy) ardResults.push(legacy);
  }

  const cardState = { count: 0 };
  for (const catalog of ardResults) await mergeArdCards(catalog, state, options, cardState);
...
export { DEFAULT_RESOLVER_MAX_BYTES };
