...
function configState(entries, siteRoot, site, warnings) {
  ...
  const basePresence = /\bbasePath\s*:/.test(text);
  const baseMatch = /\bbasePath\s*:\s*(["'])([^"']*)\1/.exec(text);
  if (basePresence && !baseMatch) {
    routingResolved = false;
    warnings.push({ code: 'next-basepath-not-literal', message: 'Next.js basePath is computed; route URLs were not guessed.', path: pathname });
  }
  ...
  if (/\b(?:async\s+)?rewrites\s*\(/.test(text)) warnings.push({ code: 'next-rewrites-not-executed', message: 'Next.js rewrites can change public routing; v0.1 preserves affected route ownership conservatively.', path: pathname });
  return { path: pathname, routingResolved, hasRewrites: /\b(?:async\s+)?rewrites\s*\(/.test(text) };
}

function appRoot(entries) {
  const roots = ['app', 'src/app'].filter(root => [...entries.values()].some(entry => entry.projectRelative === root || entry.projectRelative.startsWith(`${root}/`)));
...
function routeFromPage(projectRelative, appRootPath) {
  ...
  const rawSegments = directory.split('/').filter(Boolean);
  const segments = [];
  ...
  return { routePath: segments.length ? `/${segments.join('/')}/` : '/', dynamic: false, reason: null };
}
...
function ancestorLayouts(pagePath, entries, appRootPath, siteRoot) {
  ...
  while (directory === appRootPath || directory.startsWith(`${appRootPath}/`)) {
    for (const name of LAYOUT_NAMES) {
      const candidateRelative = `${directory}/${name}`;
      const candidatePath = siteJoin(siteRoot, candidateRelative);
...
function buildRoutes(entries, siteRoot, appRootPath, site, config, packageInfo, warnings) {
  ...
  for (const entry of entries.values()) {
    const mapped = routeFromPage(entry.projectRelative, appRootPath);
    if (!mapped) continue;
    const sourcePath = entry.path;
    const metadataDynamic = /\b(?:export\s+)?(?:async\s+)?function\s+generateMetadata\b|\bexport\s+const\s+generateMetadata\b/.test(entry.text);
    if (metadataDynamic) warnings.push({ code: 'next-computed-metadata-unmapped', message: 'generateMetadata() was detected; v0.1 does not infer metadata field ownership from executable code.', path: sourcePath });
    if (mapped.dynamic || !config.routingResolved || config.hasRewrites) {
      const evidence = [
        ...
        ...(!config.routingResolved ? ['next:basepath-unresolved'] : []),
...
function buildMetadataRoutes(entries, appRootPath, site) {
  const ownership = [];
...
export function compileNextSiteStateGraph(options = {}) {
  ...
  const machineOwnership = buildMetadataRoutes(scanned.entries, appRootPath, site);
  const ownership = [...built.ownership, ...machineOwnership].sort((a, b) => a.surfaceKey.localeCompare(b.surfaceKey));
  const pagePaths = new Set([...scanned.entries.values()].filter(entry => routeFromPage(entry.projectRelative, appRootPath)).map(entry => entry.path));
  const metadataPaths = new Set(machineOwnership.map(item => item.ownerPath));
  const machineByPath = new Map(machineOwnership.map(item => [item.ownerPath, item]));
