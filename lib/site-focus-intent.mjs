...
const EXPLICIT_HOME_LABEL = /\b(home|start|overview)\b/i;
...
function observedPrimaryNavigation(report, profile) {
  const rootUrl = cleanUrl(profile.canonicalUrl);
  const filtered = (report.navigation?.primary || []).filter(item => {
    const url = cleanUrl(item.url, profile.canonicalUrl);
    if (!url) return false;
    if (url !== rootUrl) return true;
    return EXPLICIT_HOME_LABEL.test(String(item.text || ''));
  });
  return [...new Map(filtered.map(item => [cleanUrl(item.url, profile.canonicalUrl), item])).values()];
}

function normalizeDeclaredNavigationObservation(report, profile) {
  const primary = observedPrimaryNavigation(report, profile);
  const primaryUrls = new Set(primary.map(item => cleanUrl(item.url, profile.canonicalUrl)).filter(Boolean));
  const technicalCandidates = (report.navigation?.technicalCandidates || []).filter(item => primaryUrls.has(cleanUrl(item.url, profile.canonicalUrl)));
  let siteFindings = [...(report.siteFindings || [])].filter(item => item.id !== 'wide-primary-navigation' || primary.length > 5);
  const technicalShare = technicalCandidates.length / Math.max(primary.length, 1);
  siteFindings = siteFindings.filter(item => item.id !== 'technology-led-navigation');
  if (technicalCandidates.length && technicalShare >= 0.4) siteFindings.push({
    id: 'technology-led-navigation',
    severity: 'review',
    evidence: technicalCandidates,
    message: 'A large share of primary navigation is labeled with implementation technology. Review whether user problems should own those first-order routes instead.'
  });
  return {
    ...report,
    navigation: { ...(report.navigation || {}), primary, technicalCandidates },
    metrics: {
      ...(report.metrics || {}),
      primaryNavDestinations: primary.length,
      technicalNavDestinations: technicalCandidates.length
    },
    siteFindings
  };
}
...
export function applySiteFocusIntent(report, profile, { profileSource = null } = {}) {
  ...
  report = normalizeDeclaredNavigationObservation(report, profile);
  ...
  return {
    ...
    guardrails: {
      ...
      brandIdentityHomeLinkExcludedFromDeclaredNavigation: true,
