...
const PROVIDERS = new Set(['google', 'bing', 'cloudflare', 'referrals']);
...
function aggregateGoogle(rows, reportScope) {
  ...
  for (const raw of rows) {
    ...
    aiImpressions += value;
    observed += 1;
    const page = firstText(row, ['page', 'url', 'pageUrl', 'landingPage']);
    const country = firstText(row, ['country']);
    const device = firstText(row, ['device']);
    if (page) pages.add(page);
    if (country) countries.add(country);
    if (device) devices.add(device);
  }
  const metrics = {};
  if (observed) metrics.aiImpressions = aiImpressions;
  if (pages.size) metrics.aiVisiblePages = pages.size;
  if (countries.size) metrics.aiVisibleCountries = countries.size;
  if (devices.size) metrics.aiVisibleDevices = devices.size;
  const coverage = [pages.size ? `${pages.size} page(s)` : null, countries.size ? `${countries.size} countr${countries.size === 1 ? 'y' : 'ies'}` : null, devices.size ? `${devices.size} device class(es)` : null].filter(Boolean);
  return {
    provider: 'google-search-console-generative-ai',
    status: Object.keys(metrics).length ? 'observed' : 'partial',
    metrics,
    notes: (observed ? `Aggregated ${observed} row(s) containing generative-search impression evidence${coverage.length ? ` and observed ${coverage.join(', ')}` : ''}.` : 'No recognized generative-search impression column was found; preserve as partial evidence.')
...
function aggregateCloudflare(rows) {
  const totals = {
    aiCrawlerRequests: 0,
    aiCrawlerAllowedRequests: 0,
    aiCrawlerSuccessfulRequests: 0,
    aiCrawlerUnsuccessfulRequests: 0,
    aiCrawlerReferrals: 0,
    bytesTransferred: 0
  };
  const observed = new Set();
  for (const raw of rows) {
    const row = normalizedRow(raw);
    const requests = firstNumber(row, ['aiCrawlerRequests', 'totalRequests', 'requests', 'requestCount']);
    const allowed = firstNumber(row, ['aiCrawlerAllowedRequests', 'allowedRequests', 'allowedRequestCount']);
    const successful = firstNumber(row, ['aiCrawlerSuccessfulRequests', 'successfulRequests', 'successRequests', '2xxRequests']);
    const unsuccessful = firstNumber(row, ['aiCrawlerUnsuccessfulRequests', 'unsuccessfulRequests', 'failedRequests', 'errorRequests']);
    const referrals = firstNumber(row, ['aiCrawlerReferrals', 'totalReferrals', 'referrals']);
    const bytes = firstNumber(row, ['bytesTransferred', 'bytes', 'dataTransferBytes']);
    const statusCode = firstNumber(row, ['statusCode', 'edgeResponseStatus', 'httpStatus']);
    const action = firstText(row, ['action', 'crawlerAction', 'decision']);

    if (requests != null) { totals.aiCrawlerRequests += requests; observed.add('aiCrawlerRequests'); }
    if (allowed != null) { totals.aiCrawlerAllowedRequests += allowed; observed.add('aiCrawlerAllowedRequests'); }
    else if (requests != null && action && /allow/i.test(action)) { totals.aiCrawlerAllowedRequests += requests; observed.add('aiCrawlerAllowedRequests'); }
    if (successful != null) { totals.aiCrawlerSuccessfulRequests += successful; observed.add('aiCrawlerSuccessfulRequests'); }
    else if (requests != null && statusCode != null && statusCode >= 200 && statusCode < 400) { totals.aiCrawlerSuccessfulRequests += requests; observed.add('aiCrawlerSuccessfulRequests'); }
    if (unsuccessful != null) { totals.aiCrawlerUnsuccessfulRequests += unsuccessful; observed.add('aiCrawlerUnsuccessfulRequests'); }
    else if (requests != null && statusCode != null && statusCode >= 400) { totals.aiCrawlerUnsuccessfulRequests += requests; observed.add('aiCrawlerUnsuccessfulRequests'); }
    if (referrals != null) { totals.aiCrawlerReferrals += referrals; observed.add('aiCrawlerReferrals'); }
    if (bytes != null) { totals.bytesTransferred += bytes; observed.add('bytesTransferred'); }
  }
  const metrics = Object.fromEntries([...observed].map(metric => [metric, totals[metric]]));
  return {
    provider: 'cloudflare-ai-crawl-control',
    status: Object.keys(metrics).length ? 'observed' : 'partial',
    metrics,
    notes: Object.keys(metrics).length
      ? `Aggregated ${rows.length} Cloudflare AI Crawl Control row(s). Status/action inference is used only when an explicit request count is present on the same row.`
      : 'No recognized Cloudflare AI Crawl Control metrics were found; preserve as partial evidence.'
  };
}
...
function aggregateReferrals(rows, matchers) {
  ...
  let referralObserved = 0;
  let engagedObserved = 0;
  let taskObserved = 0;
  let rowsWithReferrer = 0;
  for (const raw of rows) {
    const row = normalizedRow(raw);
    const referrer = firstText(row, ['referrer', 'source', 'sessionSource', 'trafficSource', 'domain', 'utmSource', 'firstUserSource']);
    if (referrer) rowsWithReferrer += 1;
    const matched = !referrer || matchers.some(value => referrer.toLowerCase().includes(value.toLowerCase()));
    if (!matched) continue;
    const visits = firstNumber(row, ['referrals', 'sessions', 'visits', 'entrances', 'users']);
    const engaged = firstNumber(row, ['engagedVisits', 'engagedSessions']);
    const tasks = firstNumber(row, ['taskCompletions', 'conversions', 'keyEvents', 'keyEventCount']);
    if (visits != null) { referrals += visits; referralObserved += 1; }
    else if (referrer && engaged == null && tasks == null) { referrals += 1; referralObserved += 1; }
    if (engaged != null) { engagedVisits += engaged; engagedObserved += 1; }
    if (tasks != null) { taskCompletions += tasks; taskObserved += 1; }
  }
  const metrics = {};
  if (referralObserved) metrics.referrals = referrals;
  if (engagedObserved) metrics.engagedVisits = engagedVisits;
  if (taskObserved) metrics.taskCompletions = taskCompletions;
  return {
    provider: 'referral-analytics',
    status: Object.keys(metrics).length ? 'observed' : 'partial',
    metrics,
    notes: Object.keys(metrics).length
      ? `Aggregated matching AI referral evidence. ${rowsWithReferrer ? `Filtered by: ${matchers.join(', ')}.` : 'No referrer/source dimension was present, so explicit aggregate fields were used as supplied.'}`
...
export function importVisibilityExport(provider, filePath, options = {}) {
  if (!PROVIDERS.has(provider)) throw new Error(`Unsupported provider: ${provider}. Use google, bing, cloudflare or referrals.`);
  ...
  const imported = provider === 'google'
    ...
    : provider === 'bing'
      ...
      : provider === 'cloudflare'
        ? aggregateCloudflare(source.rows)
