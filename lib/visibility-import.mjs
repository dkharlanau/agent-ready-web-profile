...
function aggregateGoogle(rows) {
  ...
  for (const raw of rows) {
    ...
    const value = firstNumber(row, ['aiImpressions', 'generativeAiImpressions', 'generativeSearchImpressions', 'impressions']);
...
export function importVisibilityExport(provider, filePath, options = {}) {
  ...
  return { snapshot, import: { provider, format: source.format, rows: source.rows.length, source: source.absolute, matchers: provider === 'referrals' ? matchers : null } };
}
