...
export function adaptA2aCard(card, sourceId, sourceUrl) {
  ...
  for (const [index, callable] of shape.interfaces.entries()) {
    add(out, 'agents', {
      sourceId,
      kind: 'agent-endpoint',
      protocol: 'A2A',
      url: callable.url,
      agentCardUrl: sourceUrl,
      interfaceIndex: index,
      protocolBinding: callable.protocolBinding || null,
      protocolVersion: callable.protocolVersion || null,
      name: card.name,
...
function ardEntryMeta(entry) {
  return {
    ...
    trustManifest: entry?.trustManifest || null
  };
}
...
export function adaptAiCatalog(payload, sourceId) {
  ...
  const cards = [];
  const issues = [];
  const specVersion = payload?.specVersion || null;
  const entries = Array.isArray(payload?.entries) ? payload.entries : [];
  if (specVersion && specVersion !== '1.0') issues.push(`Unrecognized ai-catalog specVersion: ${specVersion}`);

  for (const entry of entries) {
    if (!entry || typeof entry !== 'object') continue;
    const type = String(entry.type || '').toLowerCase();
    const meta = ardEntryMeta(entry);
    const hasUrl = typeof entry.url === 'string' && /^https:\/\//i.test(entry.url);
    const hasData = entry.data && typeof entry.data === 'object' && !Array.isArray(entry.data);
    if (hasUrl === Boolean(hasData)) {
      issues.push(`${entry.identifier || entry.displayName || 'catalog entry'} must provide exactly one usable url or data value.`);
      continue;
    }

    if (type === 'application/mcp-server-card+json') {
      cards.push({ identifier: entry.identifier || null, url: hasUrl ? entry.url : null, data: hasData ? entry.data : null, meta });
      continue;
    }

    if (type === 'application/a2a-agent-card+json') {
      if (hasData) {
        const adapted = adaptA2aCard(entry.data, sourceId, null);
        if (adapted.valid) mergeInterfaces(out, adapted.interfaces);
        else issues.push(...adapted.issues.map(issue => `${entry.identifier || 'inline A2A card'}: ${issue}`));
      } else {
        add(out, 'data', urlItem(entry.url, sourceId, { kind: 'ard-a2a-card-reference', protocol: 'ARD', artifactProtocol: 'A2A', ...meta }));
      }
      continue;
    }

    if (type === 'application/ai-skill' || type === 'application/ai-skill+md') {
      if (hasUrl) add(out, 'skills', urlItem(entry.url, sourceId, { kind: 'ard-skill', protocol: 'Agent Skills', discoveryProtocol: 'ARD', ...meta }));
      else add(out, 'data', { sourceId, kind: 'ard-inline-skill', protocol: 'ARD', inline: true, ...meta });
      continue;
    }

    if (type === 'application/ai-registry+json') {
      if (hasUrl) add(out, 'data', urlItem(entry.url, sourceId, { kind: 'ard-registry', protocol: 'ARD', ...meta }));
      else add(out, 'data', { sourceId, kind: 'ard-inline-registry', protocol: 'ARD', inline: true, ...meta });
      continue;
    }

    if (type === 'application/ai-catalog+json') {
      if (hasUrl) add(out, 'data', urlItem(entry.url, sourceId, { kind: 'ard-catalog', protocol: 'ARD', ...meta }));
      else add(out, 'data', { sourceId, kind: 'ard-inline-catalog', protocol: 'ARD', inline: true, entries: Array.isArray(entry.data?.entries) ? entry.data.entries.length : null, ...meta });
      continue;
    }

    if (hasUrl) add(out, 'data', urlItem(entry.url, sourceId, { kind: 'ard-artifact', protocol: 'ARD', ...meta }));
    else add(out, 'data', { sourceId, kind: 'ard-inline-artifact', protocol: 'ARD', inline: true, ...meta });
  }

  return {
    interfaces: out,
    cards,
    authority: AUTHORITY.ard,
    catalog: {
      specVersion,
