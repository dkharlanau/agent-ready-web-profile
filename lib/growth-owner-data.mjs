...
export const OWNER_DATA_ACTION_REQUIREMENTS = Object.freeze({
  'growth:google-generative-ai-measurement-global': Object.freeze({
    ...
    collection: Object.freeze({
      mode: 'ui-export',
      surface: 'Search Console > Performance > Generative AI (Search)',
      source: 'https://support.google.com/webmasters/answer/16984139',
      reviewedAt: '2026-09-06',
      automation: 'owner-export-required',
      note: 'Use the dedicated Generative AI performance report or its export. Do not substitute ordinary Search Analytics or guess an undocumented searchAppearance enum.'
    })
  }),
  'growth:bing-ai-citation-measurement': Object.freeze({
    ...
    collection: Object.freeze({
      mode: 'ui-export',
      surface: 'Bing Webmaster Tools > AI Performance',
      source: 'https://www.bing.com/webmasters/help/ai-performance-9f8e7d6c',
      reviewedAt: '2026-09-06',
      automation: 'owner-export-required',
      note: 'Use the authenticated AI Performance dashboard or its CSV/Excel export. Do not substitute traditional Bing search traffic or crawl data.'
    })
  }),
  'growth:google-platform-properties': Object.freeze({
    ...
    collection: Object.freeze({
      mode: 'ui-export',
      surface: 'Search Console platform properties',
      source: 'https://support.google.com/webmasters/',
      reviewedAt: '2026-09-06',
      automation: 'owner-export-required',
      note: 'Conditional opportunity only when the publisher operates a supported social/video account or channel and wants its Search footprint measured.'
    })
  }),
  'trend-owner:google-generative-ai-control-global': Object.freeze({
    ...
    collection: Object.freeze({
      mode: 'ui',
      surface: 'Search Console > Settings > Generative AI in Search',
      source: 'https://support.google.com/webmasters/answer/16908024',
      reviewedAt: '2026-09-06',
      automation: 'owner-ui-verification-required',
      note: 'Verify the authenticated property setting itself. Public crawling and ordinary Search Analytics cannot prove this owner control.'
    })
  })
});
...
function annotateOwnerGate(item) {
  const requirement = OWNER_DATA_ACTION_REQUIREMENTS[item.id];
  if (!requirement?.collection) return item;
  return {
    ...item,
    ownerDataCollection: {
...
function externalGateSummary(actions) {
  const gates = actions.filter(item => item.status === 'external-owner-data');
  return {
    count: gates.length,
    byCollectionMode: gates.reduce((acc, item) => {
      const mode = item.ownerDataCollection?.mode || 'unknown';
...
export function applyOwnerDataEvidence(plan, localOwnerData = null, { canonicalUrl, now = new Date() } = {}) {
  ...
  const classifiedActions = plan.actions.map(annotateOwnerGate).map(optionalize);
  ...
  const externalGates = externalGateSummary(actions);
  ...
  return {
    ...
    observations: {
      ...
      ownerData: {
        ...
        matchedActionIds,
        unmatchedRecordIds,
        activeExternalGates: actions
          .filter(item => item.status === 'external-owner-data')
          .map(item => ({
            actionId: item.id,
            provider: item.ownerDataCollection?.provider || null,
            kind: item.ownerDataCollection?.kind || null,
            collectionMode: item.ownerDataCollection?.mode || 'unknown',
            surface: item.ownerDataCollection?.surface || null,
            automation: item.ownerDataCollection?.automation || null,
            source: item.ownerDataCollection?.source || null,
            reviewedAt: item.ownerDataCollection?.reviewedAt || null
          })),
