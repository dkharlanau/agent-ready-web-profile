...
export function applyOwnerDataEvidence(plan, localOwnerData = null, { canonicalUrl, now = new Date() } = {}) {
  ...
  const optionalActions = plan.actions.filter(item => item.status === 'opportunity');
  let actions = plan.actions.filter(item => item.status !== 'opportunity');
  ...
  return {
    ...
    refinements: {
      ...
      ownerDataNote: 'Owner-side evidence resolves only exact external-owner-data actions with matching provider, measurement/control kind, status, value where required, and freshness. Ordinary Search Console performance cannot stand in for generative AI reporting, Bing AI citations, social/video platform properties, or authenticated inclusion controls.'
    }
  };
}
