...
export function buildSiteFocusReportFromPages(input, { canonicalUrl = input?.[0]?.url, discovery = null } = {}) {
  ...
  if (distinctNav.length > 5) siteFindings.push({ id: 'wide-primary-navigation', severity: 'review', evidence: distinctNav, message: 'Primary navigation exceeds the Cite Goose house heuristic of five destinations.' });
...
export function formatSiteFocusReport(report) {
  const lines = [
    `Cite Goose Site Focus — ${report.canonicalUrl}`,
