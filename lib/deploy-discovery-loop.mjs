...
export function planDeployDiscovery({
  ...
  const verifiedRemoved = diff.removed.filter(item => item.verifiedGone);
  const candidates = [
    ...diff.added.map(item => item.url),
    ...diff.updated.map(item => item.url),
    ...verifiedRemoved.map(item => item.url)
  ];
  const readyUrls = deployment.pass ? [...new Set(candidates)].sort() : [];
  const blockedCandidates = deployment.pass ? [] : [...new Set(candidates)].sort();
  ...
  return {
    ...
    feed,
    indexNow: {
      state: deployment.pass ? (readyUrls.length ? 'ready' : 'not-needed') : 'blocked',
      readyUrls,
      blockedCandidates,
      watchUrls,
      note: 'Ready means evidence-backed and deploy-gated for the existing IndexNow helper. It does not mean submitted, crawled or indexed.'
    },
    ownerEvidenceQueue: ownerEvidence,
    failures,
    watches,
    pass: failures.length === 0 && deployment.pass && sitemap.state === 'pass',
...
export function formatDeployDiscoveryReport(report) {
  const lines = [
    ...
    `IndexNow: ${report.indexNow.state.toUpperCase()} (${report.indexNow.readyUrls.length} ready, ${report.indexNow.watchUrls.length} watch)`,
