...
function preferredSourcesFinding(rule, input) {
  ...
  if (input.siteScope === 'hostname-root' && !rootPath) {
    return finding(rule, 'fail', `The site was declared hostname-root but its public root contains a path (${pathname}). Preferred Sources eligibility is hostname-level; classify this as subdirectory or provide the actual hostname root.`, [input.site]);
  }
  if (input.siteScope === 'subdirectory' && rootPath) {
    return finding(rule, 'fail', 'The site was declared subdirectory but the supplied public root is the hostname root. Resolve the scope declaration before applying Preferred Sources guidance.', [input.site]);
  }
