...
export function semanticWarnings(profile) {
  ...
  for (const skill of profile.agentSkills?.skills ?? []) {
    if (!skill.url.endsWith('/SKILL.md') && !skill.url.endsWith('SKILL.md')) {
      warnings.push(`Agent Skill ${skill.name} does not point directly to a SKILL.md file.`);
    }
  }

  for (const server of profile.mcp?.servers ?? []) {
    if (server.transport === 'stdio' && !server.package && !server.source) {
      warnings.push(`MCP server ${server.name} uses stdio but does not declare package or source metadata.`);
    }
  ...
  const capabilityKeys = ['web', 'data', 'retrieval', 'agentSkills', 'agentWeb', 'mcp', 'a2a'];
