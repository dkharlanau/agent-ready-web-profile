...
| Image discovery / technical image quality / attribution | `registry/image-discovery-practices.json`, `registry/image-quality-practices.json`, `docs/IMAGE-DISCOVERY-LAYER.md`, `docs/IMAGE-QUALITY-LAYER.md`, `skills/arwp-image-discovery/SKILL.md` | target image/media manifest or source assets; final generated HTML; preferred/social image metadata; responsive variants; image sitemap; visible credits and applicable `ImageObject`/IPTC metadata; GitHub Pages portfolio attribution policy | `node scripts/image-discovery-layer-test.mjs`; for target sites inventory exact bytes + intrinsic/rendered dimensions, verify format/quality changes, `srcset`/`sizes`, width/height, lazy/LCP priority, renamed references and creator/license/credit provenance; do not treat a green implementation check as indexing, field-CWV or rights-ownership proof |
...
- meaningful representative/share imagery also follows `registry/image-quality-practices.json` for byte/dimension, responsive delivery, visual-quality and attribution checks;
