...
| Internal discovery / crawl-path evidence | `docs/INTERNAL-DISCOVERY-EVIDENCE.md`, `schema/internal-discovery-report.schema.json` | `lib/internal-discovery.mjs`, `bin/arwp-internal-discovery.mjs`; bounded Site Gate cohort; rendered HTML owner/alias/link-class graph | `node scripts/internal-discovery-test.mjs`; `node bin/arwp-internal-discovery.mjs <site> --max-pages=20 --json`; never convert partial anchor coverage into orphan evidence or a graph/SEO score |
